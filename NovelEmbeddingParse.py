...
try:
        metadata = {
            "character_name": ["宋侧", "宁长久", "宁小龄", "宁擒水"],
            "chapter": "第一章：皇城的鬼"
        }
        # 1. 创建测试文档
        doc = Document(text=chapter_str, metadata=metadata)
        ...
        print("\n⏳ 正在插入索引...")
        try:
            ggraph_manager.index.insert(doc)
            print("✅ 插入操作完成（无报错）")
        ...
        print("\n⏳ 正在持久化...")
        try:
            ggraph_manager.storage_context.persist(persist_dir=ggraph_manager.persist_dir)
            print(f"✅ 持久化完成，目录: {ggraph_manager.persist_dir}")
        except Exception as e:
            print(f"❌ 持久化报错: {e}")
except Exception as e:
    print(e)
...
retriever = LLMSynonymRetriever(
    ggraph_manager.index.property_graph_store,
    similarity_top_k=5,
)
results = retriever.retrieve("宁小龄")
