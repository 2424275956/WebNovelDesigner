...
def query_role_relation(project_id, role_a, role_b):
    return SqliteDB.query_execute("SELECT relation FROM role_relation WHERE project_id = ? and role_a_name = ? and role_b_name = ?", (project_id, role_a, role_b))
...
def insert_role_model(project_id, role_name, role_json):
    SqliteDB.execute("INSERT INTO role_model (project_id, role_name, role_json) VALUES (?, ?, ?)", (project_id, str(role_name), str(role_json)))
