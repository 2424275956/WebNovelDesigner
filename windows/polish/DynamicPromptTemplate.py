...
def get_repetition_prompt_template(inputs) -> ChatPromptTemplate:
    ...
    【核心要求】：
    - 如果'文本片段'内容中，除了拟声词以外，每段内容没有完全重复的内容则直接输出'文本片段'内容。
    - 如果'文本片段'内容中，除了拟声词以外，存在完全重复的内容，将后续出现的内容清除并保证前后通顺。
    """
    user_prompt = f"""
    【文本片段】
    {inputs['polish_text']}
...
def get_original_framework_prompt_template(inputs) -> ChatPromptTemplate:
    ...
    system_template += """
    【绝对规则】：以‘待改写段落’内容为核心，必须保留完整剧情、对话与细节，必须融入设定并新增大量的性交互动作与对话。'前述剧情'与'后续剧情'仅供参考并作为时间轴，禁止改写内容跨越时间轴。
