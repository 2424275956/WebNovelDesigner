...
def get_original_framework_prompt_template(inputs) -> ChatPromptTemplate:
    ...
    3. 【禁止输出后文】禁止在改写结果末尾添加【后文衔接】的任何内容。禁止让【待改写片段】的结尾"滑入"后文剧情。
    4. 【禁止剧透前置】改写后的片段中，人物不能提前知道后文才揭示的信息，不能提前出现后文才出现的道具、地点、人物关系变化。
    5. 【禁止元评论】禁止输出"改写如下：""以下是修改后的片段"等前缀，禁止输出修改说明。
    【边界检测标准】
...
def get_polish_prompt_template(inputs) -> ChatPromptTemplate:
    ...
    system_template = str(inputs['system_prompt'])
    system_template += """
    【绝对规则】：必须以‘待润色段落’的剧情脉络与对话信息为核心。将设定融入其中，大量填充角色细节描述与人物动作对话的性交互的描绘。禁止对无关内容进行改写。
    【输出内容】
    "润色完成后的内容"
    """
    system_template = special_chars_parse(system_template)
...
def get_extra_framework_prompt_template(inputs) -> ChatPromptTemplate:
    ...
    system_template = str(inputs['system_prompt'])
    system_template += """
    【输出内容】: 必须以‘前述剧情’与‘后续剧情’为屏障与参考，通过设定与角色生成新的剧情脉络，其中必须含有大量的性交互动作与对话。禁止携带与内容无关输出
    [脉络生成的内容]
    """
    system_template = special_chars_parse(system_template)
    # 用户提示词
